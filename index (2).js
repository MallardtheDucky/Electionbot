...
function buildCommandsArray() {
    return [
        ...
        new SlashCommandBuilder()
            .setName('signup2')
            .setDescription('Sign up for an election (States M-W)')
            .addStringOption(option =>
                option.setName('state')
                    .setDescription('The state you want to run in')
                    .setRequired(true)
        ...
        new SlashCommandBuilder()
            .setName('poll')
            .setDescription('Create a 24-hour poll by office, state, and optional party; winner gets 8 points')
            .addStringOption(option =>
                option.setName('office')
                    ...
                    .addChoices(
                        ...
                        { name: 'District 6', value: 'District 6' },
                        { name: 'District 7', value: 'District 7' }
                    ))
            .addStringOption(option =>
                option.setName('state')
            ...
            .addStringOption(option =>
                option.setName('party')
                    .setDescription('The party to poll (optional)')
                    .setRequired(false)
                    .addChoices(
                        { name: 'Republicans', value: 'Republicans' },
                        { name: 'Democrats', value: 'Democrats' },
        ...
        new SlashCommandBuilder()
            .setName('poll2')
            .setDescription('Create a simulated poll (States M-W)')
            .addStringOption(option =>
                option.setName('office')
                    .setDescription('The office to poll')
                    .setRequired(true)
        ...
        new SlashCommandBuilder()
            .setName('list_signups2')
            .setDescription('List all candidates by phase, race, and party (States M-W)')
            .addStringOption(option =>
                option.setName('state')
    ...
    try {
        if (interaction.isChatInputCommand()) {
            switch (interaction.commandName) {
                ...
                case 'poll':
                case 'poll2':
                    await handleSimulatedPoll(interaction);
                    break;
                case 'list_signups':
                case 'list_signups2':
...
async function handleSimulatedPoll(interaction) {
    await interaction.deferReply();
    ...
    try {
        ...
        console.log(`Creating simulated poll: Office=${office}, State=${state}, Party=${party || 'All'}`);

        // Get President Ideology sheet data
        const ideologyData = await getSheetData('President Ideology');
        if (!ideologyData || ideologyData.length === 0) {
            await interaction.editReply('No data available in President Ideology sheet');
            return;
        }

        const ideologyRecords = parseSheetData(ideologyData);
        
        // Determine which sheet to use for candidates
        const phase = cycle.phase.toLowerCase();
        let candidateSheetName = 'All Signups';
        if (office.toLowerCase() === 'president') {
            candidateSheetName = 'President Signups';
        } else if (['general campaign', 'general election'].includes(phase)) {
            candidateSheetName = 'All Winners';
        }

        const candidateData = await getSheetData(candidateSheetName);
        if (!candidateData || candidateData.length === 0) {
            await interaction.editReply(`No data available in ${candidateSheetName}`);
            return;
        }

        const candidateRecords = parseSheetData(candidateData);

        // Filter candidates based on office, state, and party
        let candidates = candidateRecords.filter(r => {
            const matchesOffice = r.Office?.toLowerCase() === office.toLowerCase();
            const matchesState = r.States?.toLowerCase() === state.toLowerCase();
        ...
        if (candidates.length === 0) {
            await interaction.editReply(`No candidates found for ${office} in ${state}${party ? ` (${party})` : ''}`);
            return;
        }

        // Simulate the poll
        const pollResults = await simulatePoll(office, state, candidates, ideologyRecords);
        
        // Generate random NPC count between 10,000 and 100,000
        const npcCount = Math.floor(Math.random() * 90001) + 10000;

        // Create result embed
        const embed = new EmbedBuilder()
            .setTitle(`📊 Simulated Poll Results for ${office} in ${state}${party ? ` (${party})` : ''}`)
            .setColor(0x00ff00)
            .setDescription(pollResults.map(result => 
                `**${result.name}** (${result.party}): ${result.percentage.toFixed(1)}%`
            ).join('\n'))
            .setFooter({ text: `Simulated with ${npcCount.toLocaleString()} NPCs | Phase: ${cycle.phase}` });

        await interaction.editReply({ embeds: [embed] });

    } catch (error) {
        console.error('Error in simulated poll:', error);
        await interaction.editReply(`Error creating simulated poll: ${error.message}`);
    }
}

async function simulatePoll(office, state, candidates, ideologyRecords) {
    const isHouseOrPresident = office.toLowerCase() === 'president' || office.toLowerCase().includes('district');
    
    let pollResults = [];

    if (isHouseOrPresident) {
        // For House/President: Use state lean from President Ideology sheet
        let stateLean = { republican: 33, democrat: 33, other: 34 }; // Default

        if (office.toLowerCase() === 'president') {
            // Find state in President Ideology sheet
            const stateRecord = ideologyRecords.find(r => 
                r.States?.toLowerCase() === state.toLowerCase()
            );
            
            if (stateRecord) {
                stateLean.republican = parseFloat(stateRecord.REPUBLICAN) || 33;
                stateLean.democrat = parseFloat(stateRecord.DEMOCRAT) || 33;
                stateLean.other = parseFloat(stateRecord['OTHER/UNDECIDED']) || 34;
            }
        } else {
            // For House districts, find the specific district
            const districtRecord = ideologyRecords.find(r => 
                r['HOUSE DISTRICTS']?.toLowerCase() === office.toLowerCase()
            );
            
            if (districtRecord) {
                stateLean.republican = parseFloat(districtRecord['HOUSE REPUBLICAN']) || 33;
                stateLean.democrat = parseFloat(districtRecord['HOUSE DEMOCRAT']) || 33;
                stateLean.other = parseFloat(districtRecord['HOUSE OTHER/UNDECIDED']) || 34;
            }
        }

        // Calculate poll results based on state lean + candidate factors
        for (const candidate of candidates) {
            ...
            let basePercentage = stateLean.other; // Default to other/undecided
            if (candidate.Party?.toLowerCase() === 'republicans') {
                basePercentage = stateLean.republican;
            } else if (candidate.Party?.toLowerCase() === 'democrats') {
                basePercentage = stateLean.democrat;
            }
            
            // Modify by candidate factors
            const candidateModifier = (points * 0.1) + (stamina * 0.05) - (corruption * 0.1);
            const finalPercentage = Math.max(0, basePercentage + candidateModifier + (Math.random() * 10 - 5));
        ...
        // For Senate/Governor: Based purely on candidate stats
        const totalScores = [];
        ...
        for (const candidate of candidates) {
            ...
            const score = points + (stamina / 10) - (corruption / 10) + (Math.random() * 20 - 10);
            totalScores.push(Math.max(0, score));
        }
        
        const totalSum = totalScores.reduce((a, b) => a + b, 0);
        
        for (let i = 0; i < candidates.length; i++) {
            const percentage = totalSum > 0 ? (totalScores[i] / totalSum) * 100 : 100 / candidates.length;
            pollResults.push({
    ...
    const totalPercentage = pollResults.reduce((sum, result) => sum + result.percentage, 0);
    ...
    return pollResults.sort((a, b) => b.percentage - a.percentage);
}
            ...
            .addFields([
                {
                    ...
                    value: [
                        ...
                        '`/poll` - Create a 24-hour poll by office, state, and optional party; winner gets 8 points',
